...
if missing:fail(f'dotace.json: chybí povinné roky {sorted(missing)}')

required_areas={
  'Kultura','Kultura a volnočasové aktivity','Volnočasové aktivity dětí a mládeže',
  'Sportovní výchova mládeže','Sociální oblast','Sport dospělí a dorost','Zvelebování vzhledu MČ Praha 8'
}
areas={str(g.get('area') or '').strip() for g in grants}
missing_areas=required_areas-areas
if missing_areas:fail(f'dotace.json: chybí oblasti {sorted(missing_areas)}')
...
for year,minimum in minimums.items():
  if counts.get(year,0)<minimum:fail(f'dotace.json: rok {year} má jen {counts.get(year,0)} záznamů, bezpečné minimum je {minimum}')

modern_social={int(y) for y in meta.get('modernSocialLoadedYears',[])}
if not {2019,2020,2021}.issubset(modern_social):fail(f'dotace.json: chybí ověřené sociální roky 2019–2021; načteno {sorted(modern_social)}')
legacy_sport={int(y) for y in meta.get('legacySportLoadedYears',[])}
if not {2014,2015}.issubset(legacy_sport):fail(f'dotace.json: chybí ověřený sport dospělých 2014/2015; načteno {sorted(legacy_sport)}')
extra_unmatched=int(meta.get('extraordinaryUnmatched',meta.get('individualUnmatched',0)) or 0)
if extra_unmatched!=0:fail(f'dotace.json: zůstává {extra_unmatched} nevyřešených kandidátů mimořádných dotací')
...
def invalid_recipient(value):
  ...
  bad={'celkem','součet','soucet','celkem přiděleno','celkem prideleno','součet všech projektů','soucet vsech projektu','součet projektů','soucet projektu','žadatel','zadatel','příjemce','prijemce','organizace','název organizace','nazev organizace'}
...
for i,g in enumerate(grants):
  ...
  try:year=int(g.get('year') or 0)
  except Exception:year=0

  # Formát názvu příjemce hlídáme přísně od roku 2014. U 2008–2013 jsou
  # zachované historické DOC/text exporty, kde `|` může být pozůstatek tabulky;
  # tyto řádky necháváme beze změny, aby QA sama nesnižovala historické počty.
  if year>=STRICT_RECIPIENT_FROM:
    if invalid_recipient(recipient):fail(f'dotace.json: záznam {i} má neplatného příjemce {recipient!r}')
  ...
  try:amount=float(g.get('approvedCzk') or 0)
    ...
    fail(f'dotace.json: záznam {i} má neplatnou částku');amount=0
  if amount<=0:fail(f'dotace.json: záznam {i} má nekladnou schválenou částku')
  if not (g.get('sourcePage') or g.get('sourceFile') or g.get('resolutionUrl')):fail(f'dotace.json: záznam {i} nemá zdroj')
...
if errors:
  ...
  for n,message in enumerate(errors,1):print(f'  {n:02d}. {message}')
  raise RuntimeError(f'dotace.json: QA našla {len(errors)} chyb; viz kompletní seznam výše')

print(f'✅ Dotace QA: {len(grants)} záznamů · roky {sorted(years,reverse=True)} · programové dotace 2026 {round(current_program_total):,} Kč · mimořádné dotace {len(extraordinary)} záznamů / {round(extra_total):,} Kč · staré formátové anomálie 2008–2013 zachovány: {legacy_format_rows}')
