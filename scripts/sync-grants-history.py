...
# Sociální výsledky 2025 mají strojově čitelnou XLSX přílohu usnesení. Starší roky
# jsou na webu převážně jen v PDF; ty budeme doplňovat samostatně po bezpečném PDF parseru.
SOURCES.append({
...
def parse_flexible(source,file_url,blob):
  ...
  for row in rows[hi+1:]:
    ...
    grants.append({'year':source['year'],'area':source['area'],'type':'programová','recipient':name,'ico':ico_value,
