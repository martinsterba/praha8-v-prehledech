...
SPECIAL_2022={
  'Kultura':{'page':'https://www.praha8.cz/appo/usn/676?usn=4l5GcSBZF3Dl0Si8wY3pbxpl4IKg%3D%3D','publicPage':'https://www.praha8.cz/Granty-Kultura-2022','resolutionId':'Usn RMC 0192/2022','resolutionDate':'2022-04-11','decisionBody':'Rada','multi':True,'required':True},
  'Volnočasové aktivity dětí a mládeže':{'page':'https://www.praha8.cz/appo/usn/676?usn=Ly63W4CAfigftrdhEBFrrA%3D%3D','publicPage':'https://www.praha8.cz/Granty-Volnocasove-nesportovni-aktivity-2022','resolutionId':'Usn RMC 0193/2022','resolutionDate':'2022-04-11','decisionBody':'Rada','multi':True,'required':False},
  'Sportovní výchova mládeže':{'page':'https://www.praha8.cz/appo/usn/676?usn=KHBuc6sYsovNITpB3pbxpl4b3pbxpl4sxw%3D%3D','publicPage':'https://www.praha8.cz/Granty-Sportovni-vychova-mladeze-2022','resolutionId':'Usn RMC 0203/2022','resolutionDate':'2022-04-20','decisionBody':'Rada','multi':True,'required':False},
  'Sociální oblast':{'page':'https://www.praha8.cz/appo/usn/676?usn=Nb4tsOhTPlKwkt7TVty2KA%3D%3D','publicPage':'https://www.praha8.cz/Dotace-v-socialni-oblasti-2022','resolutionId':'Usn RMC 0244/2022','resolutionDate':'2022-05-04','decisionBody':'Rada','multi':True,'required':False},
}
...
def build_sources():
  ...
  # Mikrogranty jsou samostatná dotační větev. Rok 2016 má výsledky v DOCX.
...
def discover_files(source):
  ...
  for label,href in html_links(source['page']):
...
def parse_rows(source,file_url,rows,parser_name):
  ...
  if hi is None:
    # starší mikrograntové tabulky mívají jednodušší záhlaví
...
def main():
  ...
  # Klíčová oprava: nahrazujeme jen úspěšně načtenou kombinaci rok+oblast.
  # Neúspěšný parser tak už nesmaže dříve publikovaná data celého ročníku.
  ...
  payload['schema']=6; payload['updated']=datetime.now(timezone.utc).isoformat(); payload['sources']=old_sources+source_meta; payload['grants']=combined
  payload['meta']={**payload.get('meta',{}),'records':len(combined),'years':years,'areas':sorted({g.get('area','') for g in combined if g.get('area')}),'warnings':all_warnings,'ares':ares_qa,'historyCounts':counts,'historyLoadedKeys':[f'{y}|{a}' for y,a in sorted(loaded_keys,key=lambda x:(-x[0],x[1]))],'note':'Historické dotace se načítají z oficiálního rozcestníku Granty a dotace a z výsledkových příloh. Neúspěch jednoho starého formátu nemaže dříve publikovaná data. Zahrnuta je i samostatná větev Sport dospělí a dorost a dostupné mikrogranty na zvelebování vzhledu MČ Praha 8.'}
